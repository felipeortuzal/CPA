...
const modes = [
  { mode: 'quick10' as const, title: 'Simulado 10', description: '10 questões na proporção oficial dos macrotemas.', icon: Zap, detail: '30 min' },
  { mode: 'quick20' as const, title: 'Simulado 20', description: '20 questões na proporção oficial dos macrotemas.', icon: FileText, detail: '60 min' },
...
export function SimulationsPage() {
  ...
  async function load() {
    ...
    setAttempts(attemptRows); setSimulations(simulationRows); setSeen(seenIds)
  }
  useEffect(() => { void load() }, [])
  ...
  return <div className="mx-auto max-w-6xl space-y-7">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div><div className="mb-2 flex items-center gap-2"><Badge>Modo Prova</Badge><span className="text-sm text-slate-500">Edital ANBIMA v1.4 · 28/05/2026</span></div><h1 className="text-3xl font-black tracking-tight sm:text-4xl">Simulados CPA</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">Treine em uma interface sem dicas nem correção durante a prova. O modo completo replica quantidade, tempo e corte oficiais da CPA.</p></div>
    ...
    <Card className="overflow-hidden p-0">
      <div className="bg-slate-950 p-6 text-white sm:p-8"><div className="flex flex-wrap items-center gap-2"><Badge>CPA oficial</Badge><span className="text-xs text-slate-400">50 questões</span></div><h2 className="mt-4 text-2xl font-black">Modo Prova CPA</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Distribuição: 10 questões do Tema 1, 20 do Tema 2, 15 do Tema 3 e 5 do Tema 4.</p><div className="mt-5 flex flex-wrap gap-5 text-sm"><span className="inline-flex items-center gap-2"><Clock3 className="h-4 w-4 text-emerald-300"/>2h30</span><span>Corte: <strong>35/50</strong></span><span>Sem feedback durante a prova</span></div><Button className="mt-6" disabled={busy !== null || Boolean(active)} onClick={() => void start('official_exam')}>{busy === 'official_exam' ? 'Gerando...' : 'Iniciar prova completa'} <ArrowRight className="h-4 w-4"/></Button></div>
    </Card>

    <div className="grid gap-4 md:grid-cols-2">{modes.map(({mode,title,description,icon:Icon,detail}) => <Card key={mode} className="flex flex-col"><div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-emerald-400/15 text-emerald-700 dark:text-emerald-300"><Icon className="h-5 w-5"/></div><div><h2 className="font-bold">{title}</h2><p className="mt-1 text-sm leading-6 text-slate-500">{description}</p><p className="mt-2 text-xs font-semibold text-slate-400">{detail}</p></div></div><div className="mt-auto pt-5"><Button variant="secondary" disabled={busy !== null || Boolean(active)} onClick={() => void start(mode)}>{busy === mode ? 'Gerando...' : 'Iniciar'} <ArrowRight className="h-4 w-4"/></Button></div></Card>)}</div>

    <Card><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="font-bold">Simulado por tema</h2><p className="mt-1 text-sm text-slate-500">Até 20 questões focadas em um único macrotema. Não há corte oficial neste modo.</p></div><div className="flex flex-col gap-2 sm:flex-row"><select value={theme} onChange={(event) => setTheme(event.target.value as '1'|'2'|'3'|'4')} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm dark:border-white/10 dark:bg-white/5">{Object.entries(themeLabels).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select><Button disabled={busy !== null || Boolean(active)} onClick={() => void start('theme')}>{busy === 'theme' ? 'Gerando...' : 'Iniciar por tema'}</Button></div></div></Card>
    ...
    <p className="text-xs leading-5 text-slate-500">Os modos de treino usam o banco autoral local. Apenas o Modo Prova CPA aplica o corte oficial de 35/50. O histórico é salvo neste navegador.</p>
  </div>
}
