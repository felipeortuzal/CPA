...
export function ReviewSessionPage(){
  ...
  function advance(){setRevealed(false);setCompleted((value)=>value+1);setIndex((value)=>value+1)}
  ...
  if(!current)return <div className="mx-auto max-w-3xl p-4 sm:p-8"><Card><div className="py-12 text-center"><CheckCircle2 className="mx-auto h-10 w-10 text-emerald-500"/><h1 className="mt-3 text-2xl font-black">Sessão concluída</h1><p className="mt-2 text-sm text-slate-500">{completed} item(ns) revisados em um bloco estimado de {estimate} min.</p><div className="mt-6 flex justify-center gap-2"><Link to="/revisao"><Button>Voltar à Central</Button></Link><Link to="/flashcards"><Button variant="secondary">Flashcards</Button></Link></div></div></Card></div>

  const error=current.errorId?errors.find((row)=>row.id===current.errorId):null
  const question=error?cpaQuestionMap.get(error.sourceId):null
    ...
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      <Card className="overflow-hidden p-0">
        ...
        {current.type==='flashcard'?<div className="p-6">
          ...
          {!revealed?<Button className="mt-5 w-full" onClick={()=>setRevealed(true)}>Mostrar resposta</Button>:<div className="mt-5 grid gap-2 sm:grid-cols-4">{(['again','hard','good','easy'] as FlashcardRating[]).map((rating)=><button key={rating} onClick={()=>void rate(rating)} className="rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold hover:border-emerald-400 hover:bg-emerald-400/5 dark:border-white/10">{ratingLabel[rating]}</button>)}</div>}
        </div>:null}

        {current.type==='error'?<div className="p-6"><h2 className="text-lg font-black leading-7">{error?.prompt??current.title}</h2>{error?<div className="mt-5 grid gap-3 md:grid-cols-2"><div className="rounded-xl bg-rose-400/10 p-4"><p className="text-xs font-bold text-rose-600">Sua última resposta</p><p className="mt-1 text-sm">{error.selectedAnswer??'—'}</p></div><div className="rounded-xl bg-emerald-400/10 p-4"><p className="text-xs font-bold text-emerald-600">Correta</p><p className="mt-1 text-sm">{error.correctAnswer??'—'}</p></div></div>:null}{question?<p className="mt-5 text-sm leading-6 text-slate-500">{question.explanation}</p>:null}<div className="mt-6 flex flex-wrap gap-2"><Link to={current.route??'/questoes'}><Button variant="secondary"><RotateCcw className="h-4 w-4"/>Refazer questão</Button></Link><Button onClick={()=>void resolveError()}><CheckCircle2 className="h-4 w-4"/>Entendi o erro</Button></div></div>:null}

        {current.type==='content'?<div className="p-6"><h2 className="text-xl font-black leading-8">{current.title}</h2><p className="mt-3 text-sm leading-6 text-slate-500">{current.reason}</p><div className="mt-6 flex flex-wrap gap-2">{current.route?<Link to={current.route}><Button variant="secondary"><ExternalLink className="h-4 w-4"/>{current.route.startsWith('/questoes')?'Treinar este PD':'Abrir conteúdo'}</Button></Link>:null}{current.route?.startsWith('/conteudos/')?<Button onClick={()=>void reviewContent()}><CheckCircle2 className="h-4 w-4"/>Revisado</Button>:<Button variant="secondary" onClick={()=>{setIndex((value)=>value+1);setRevealed(false)}}>Pular por agora</Button>}</div></div>:null}
      </Card>

      <div className="flex items-center justify-between"><Button variant="secondary" disabled={index===0} onClick={()=>{setIndex((value)=>Math.max(0,value-1));setRevealed(false)}}><ArrowLeft className="h-4 w-4"/>Anterior</Button><button onClick={()=>{setIndex((value)=>value+1);setRevealed(false)}} className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-emerald-600">Pular <ArrowRight className="h-4 w-4"/></button></div>
    </div>
  </div>
}
